...
const UserVerificationField = ({optionsList=options}: Props) => {
  ...
  return (
    <div ref={containerRef} className='relative w-full select-none'>
        <div 
            className='h-[35] xxs:h-[42px] xs:h-[48px] border-[0.5px] border-black/20 dark:border-white/20 focus:border-primarylight/50 dark:focus:border-secondary/50 text-base text-black 
            ...
            items-center justify-between gap-4 xxs:gap-6'
        >
            <div 
                onClick={() => setIsOpen(!isOpen)}
                role='button' 
                className='cursor-pointer select-none flex items-center justify-between w-full sm:w-[50%] h-full rounded-tl-sm rounded-bl-sm bg-gray-100'>
                <span className='min-w-[200px] max-w-full text-nowrap text-black/70 dark:text-white/70 px-1 xxs:pl-4 py-1 xxs:py-2'>{selected?.label || options[0].label}</span>
                <div className='bg-gray-100 w-[32px] h-full flex items-center justify-center ml-2 border-r-[0.5px] border-black/20 dark:border-white/20'>
                    <FaChevronDown aria-label='Toggle dropdown' className={`text-black/60 dark:text-white/60 ${openAbove ? 'rotate-180' : ''}`} />
                </div>
            </div>

            <div className='bg-white w-full sm:w-[50%] h-full rounded-md px-1 xxs:pl-4 py-1 xxs:py-2'>
                Social Icons
            </div>
        </div>
